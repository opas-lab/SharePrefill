...
def main():
    ...
    # Add templates
    print(f"[INFO] Using template {args.model_template_type}")
    ...
    try:
        ...
        """
        print(f"[INFO] Running command {repr(command)}")
    ...
    print(f"[INFO] Prepare {args.task} with lines: {args.num_samples} to {save_file}")
    print(f"[INFO] Used time: {round((time.time() - start_time) / 60, 1)} minutes")
